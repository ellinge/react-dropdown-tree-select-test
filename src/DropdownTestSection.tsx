...
export const DropdownTestSection: React.FunctionComponent<DropdownTestSectionProps> = (props) => {
  ...
  const { Type, target, header, description, dummyA, dummyB, onChange,
    onAction, onNodeToggle, children, hideDefaultCheckboxes } = props;
  ...
  return (<div className={`dropdown-section ${show ? "visible" : "hidden"}`}>
    ...
    {show && <div>
        {!hideDefaultCheckboxes && <div className="flex-wrap">
          <Type data={dataMulti} onChange={onChange} onAction={onAction}
            onNodeToggle={onNodeToggle} label={`#${target}`} />
          <Type data={dataMulti} radioSelect onChange={onChange} onAction={onAction}
            onNodeToggle={onNodeToggle} label={`#${target}`} />
          <Type data={dataSingle} simpleSelect onChange={onChange} onAction={onAction}
            onNodeToggle={onNodeToggle} label={`#${target}`} />
        </div>}
        {children}
      </div>}
